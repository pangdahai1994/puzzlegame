...
public class GameFrame2 extends ZFrame{
    ...
    static ZFrame zf;
    public GameFrame2(String ip,String port) throws Exception{
        ...
        tasker=new GameTasker2(ip,port);
        tasker1=new Timer();
        tasker2=new Timer();
    ...
    static class counttask extends java.util.TimerTask{ 
    	public void run() {
    		System.out.print(tasker.changing());
            if (!tasker.changing())
				try {
					tasker.fresh();
    		...
    		if (x1==101) x1=1;
			 ...
			 if((!changing)&&(tasker.changeuseful)&&(mousedown)&&(tasker.mousex(mousedownx)>=0)&&(tasker.mousey(mousedowny)>=0)){
		        	if((mousex-mousedownx>=20)&&(tasker.mousex(mousedownx)<8)){
		        		try {
							tasker.changedh2(tasker.mousex(mousedownx), tasker.mousey(mousedowny), tasker.mousex(mousedownx)+1, tasker.mousey(mousedowny));
						} catch (RemoteException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
		        	...
		        	if((mousey-mousedowny>=20)&&(tasker.mousey(mousedowny)<8)){
		        		mousedown=false;
		        		try {
							tasker.changedh2(tasker.mousex(mousedownx), tasker.mousey(mousedowny), tasker.mousex(mousedownx), tasker.mousey(mousedowny)+1);
						} catch (RemoteException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
